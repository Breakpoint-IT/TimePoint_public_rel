...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $reset) {
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        ...
        if (strlen($password) < 8) {
            ...
            $successMessage = 'Das Passwort wurde geaendert. Sie koennen sich jetzt anmelden.';
            $reset = null;
        }
    }
}
...
<head>
    ...
    <title>Passwort zuruecksetzen - TimePoint</title>
    ...
    <div class="card w-full max-w-md bg-base-100 shadow-2xl">
        <div class="card-body">
            <div class="flex flex-col items-center mb-4">
                ...
                <h1 class="card-title text-2xl">Passwort zuruecksetzen</h1>
            </div>
                ...
                <form method="post" class="space-y-4">
                    ...
                    <div class="form-control">
                        <label class="label" for="password_confirm">
                            <span class="label-text">Passwort bestaetigen</span>
                        </label>
                        <input type="password" id="password_confirm" name="password_confirm" class="input input-bordered" minlength="8" required>
                    </div>
                    <button type="submit" class="btn btn-primary w-full">
                        <i class="fas fa-key mr-2"></i>Passwort speichern
                    </button>
                </form>
            <?php endif; ?>

            <div class="text-center mt-4">
                <a href="login.php" class="link link-primary text-sm">Zum Login</a>
            </div>
        </div>
    </div>
</body>
