...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_smtp_settings'])) {
    try {
        saveSmtpSettings([
            ...
            'encryption' => $_POST['smtp_encryption'] ?? 'starttls',
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['send_smtp_test'])) {
    ...
    if (!filter_var($testRecipient, FILTER_VALIDATE_EMAIL)) {
        ...
        try {
            ...
            $error = "SMTP-Test fehlgeschlagen: " . $e->getMessage();
        }
    }
}

if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_legal_visibility'])) {
    $showPrivacyLink = isset($_POST['show_privacy_link']);
    $showImprintLink = isset($_POST['show_imprint_link']);

    try {
        setAppSetting('show_privacy_link', $showPrivacyLink ? '1' : '0');
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_user'])) {
    ...
    if (empty($username) || empty($password) || empty($email) || empty($role) || empty($department_id) || empty($regelarbeitszeit)) {
        ...
        try {
            ...
            $stmt = $conn->prepare("INSERT INTO users (username, password, email, role, department_id, supervisor_id, regelarbeitszeit, ueberstunden, force_password_change) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$username, $hashedPassword, $email, $role, $department_id, $supervisor, $regelarbeitszeit, $ueberstunden, $forcePasswordChange]);
                            ...
                            <tbody>
                                <?php foreach ($users as $user): ?>
                                    <tr>
                                        ...
                                        <td><?= htmlspecialchars((string)($user->department_name ?? '')) ?></td>
                                        <td><?= htmlspecialchars((string)($user->supervisor_name ?? '')) ?></td>
