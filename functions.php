...
$workingHoursThisMonth = max(0, $workingHoursThisMonth - (countUserBookedHolidays($currentMonthStart, $currentMonthEnd, $user_id) * $userRegularWorkingHours));
...
function countUserBookedHolidays($startDate, $endDate, $userId)
{
    global $conn;
    ...
    foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
        $datum = $row['datum'];
        if (date('N', strtotime($datum)) < 6 && !istFeiertag($datum)) {
            $bookedHolidays++;
        }
    }

    return $bookedHolidays;
}
...
foreach ($records as $record) {
    if (in_array(trim($record['beschreibung'] ?? ''), ['Feiertag', 'Urlaub'], true)) {
        continue;
    }
    if (empty($record['endzeit'])) {
        continue;
    }
    ...
    $gesamtstunden = max(0, ($endzeit->getTimestamp() - $startzeit->getTimestamp()) / 3600);
    $pause = is_numeric($record['pause']) ? ($record['pause'] / 60) : 0;
    $arbeitstunden = max(0, $gesamtstunden - $pause);
...
$totalMinutesThisMonthFromRecords = $totalMinutesThisMonthFromRecords ?? 0;
$totalHoursThisMonthFromRecords = $totalMinutesThisMonthFromRecords / 60;
...
$totalMinutesThisYearFromRecords = $totalMinutesThisYearFromRecords ?? 0;
$totalHoursThisYearFromRecords = $totalMinutesThisYearFromRecords / 60;

// 2. Expected working hours of the year
$firstDayOfTheYear = "{$currentYear}-01-01";
$lastDayOfTheYear = date('Y-m-d');
$workingHoursThisYear = getWorkingHours($firstDayOfTheYear, $lastDayOfTheYear, $userRegularWorkingHours);
$workingHoursThisYear = max(0, $workingHoursThisYear - (countUserBookedHolidays($firstDayOfTheYear, $lastDayOfTheYear, $user_id) * $userRegularWorkingHours));
...
$expectedHoursThisWeek = max(0, $expectedHoursThisWeek - (countUserBookedHolidays($firstDayOfWeek, $lastWorkingDayOfWeek, $user_id) * $userRegularWorkingHours));
...
function updateTimeRecord($conn, $id, $column, $data) {
    ...
    try {
        $oldRecord = tpAuditFetchTimeRecord((int)$id);
        if (!$oldRecord || (int)$oldRecord['user_id'] !== (int)$_SESSION['user_id']) {
        ...
        $stmt->bindParam(':user_id', $_SESSION['user_id'], PDO::PARAM_INT);
        $stmt->execute();
        tpAuditRecord('update', 'zeiterfassung', (int)$id, (int)$_SESSION['user_id'], $oldRecord, tpAuditFetchTimeRecord((int)$id), '');
        $conn->commit();
    } catch (PDOException $e) {
