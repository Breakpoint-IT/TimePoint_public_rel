...
$stmt = $conn->prepare('SELECT regelarbeitszeit, ueberstunden, vacation_days_per_year FROM users WHERE id = :user_id');
$stmt->bindParam(':user_id', $user_id, PDO::PARAM_INT);
$stmt->execute();
$userData = $stmt->fetch(PDO::FETCH_ASSOC);
$userRegularWorkingHours = $userData['regelarbeitszeit'] ?? 8;
$previousOvertime = $userData['ueberstunden'] ?? 0.0;
$vacationDaysPerYear = (int)($userData['vacation_days_per_year'] ?? 30);
...
$overHoursThisWeek = $overHoursThisWeek ?? ($totalHoursThisWeek - (5 * $userRegularWorkingHours));
...
$stmt->execute();
$absenceRows = $stmt->fetchAll(PDO::FETCH_ASSOC);
$vacationDaysTaken = 0;
$sickDaysTaken = 0;
foreach ($absenceRows as $absenceRow) {
    if ($absenceRow['type'] === 'Urlaub') {
        $vacationDaysTaken = (int)$absenceRow['days'];
    } elseif ($absenceRow['type'] === 'Krank') {
        $sickDaysTaken = (int)$absenceRow['days'];
    }
}
$vacationDaysRemaining = max(0, $vacationDaysPerYear - $vacationDaysTaken);
...
$productivityScore = $workingHoursThisMonth > 0 ? min(100, ($totalHoursThisMonth / $workingHoursThisMonth) * 100) : 0;

// Calculate weekly productivity score
$weeklyExpectedHours = $expectedHoursThisWeek ?? (5 * $userRegularWorkingHours);
$weeklyProductivityScore = $weeklyExpectedHours > 0 ? min(100, ($totalHoursThisWeek / $weeklyExpectedHours) * 100) : 0;
    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="mb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div class="stat bg-gradient-to-br from-emerald-600 to-emerald-700 rounded-box shadow-lg">
                    <div class="stat-figure opacity-70">
        ...
        <div class="mb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div class="stat bg-gradient-to-br from-blue-500 to-blue-600 rounded-box shadow-lg">
                    ...
                    <div class="stat-value"><?= calculateRemainingHours($totalHoursThisWeek, $weeklyExpectedHours) ?></div>
                </div>
