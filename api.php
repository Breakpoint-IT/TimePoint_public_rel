...
function createNewWorkEntry($conn, $user_id) {
    ...
    try {
        ...
        tpAuditRecord('create_api', 'zeiterfassung', (int)$lastId, (int)$user_id, null, $newEntry, '');
        $conn->commit();
...
function setEndzeit($conn, $input, $user_id) {
    ...
    try {
        ...
        if ($stmt->execute()) {
            tpAuditRecord('end_api', 'zeiterfassung', (int)$id, (int)$user_id, $oldRecord, tpAuditFetchTimeRecord((int)$id), '');
            $conn->commit();
...
function deleteTimeEntry($conn, $user_id, $entry_id) {
    try {
        ...
        $stmt->bindParam(':user_id', $user_id, PDO::PARAM_INT);
        $stmt->execute();
        tpAuditRecord('delete_api', 'zeiterfassung', (int)$entry_id, (int)$user_id, $entry, null, '');
        $conn->commit();
